#' Fit the Stan SEEIQR hierarchical multiregion model (prediction for November)
...
fit_seeiqr <- function(daily_cases,  # will be a matrix
  ...
  initf <- function(stan_data) {
    ...
    init <- list(R0 = R0, f2 = f2, f3 = f3, f4 = f4, f5 = f5, f6 = f6, phi = phi, sampFrac_1 = sampFrac_1, sampFrac_2 = sampFrac_2, sampFrac_3 = sampFrac_3, sampFrac_4 = sampFrac_4)
    init
  }
  pars_save <- c("R0", "f2", "f3", "f4", "f5", "f6", "phi", "lambda_d",  "sampFrac_1", "sampFrac_2", "sampFrac_3", "sampFrac_4", "y_rep")###
  ...
  list(
    ...
    f2_prior = f2_prior, f3_prior = f3_prior, f4_prior = f4_prior, f5_prior = f5_prior, f6_prior = f6_prior, obs_model = obs_model, state_0 = state_0,
    ...
    f6_prior_beta_shape2 = beta_shapef62, stan_data = stan_data
  )
}
